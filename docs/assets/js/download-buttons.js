$.getJSON('https://api.github.com/repos/camalot/xtc/releases?callback=?', function (response) {
  if (response) {
    $('.release-name').html(response.data[0].name)
    response.data[0].assets.forEach(function(asset) {
      setLink('.txc-osx', '.dmg', asset)
      setLink('.xtc-win', '.exe', asset)
      setLink('.xtc-linux', '.AppImage', asset)
    })
